...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, focal=False, gamma=None, alpha_t=None):
        ...
        if alpha_t is not None:
            empty_weight = torch.tensor(alpha_t)
        else:
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, gamma=1.1, alpha=1, log=True):
        ...
        if not self.focal:
            ...
            assert self.gamma is not None, "focal loss requires gamma parameter"
            focal = focal_loss(self.alpha_t, self.gamma, device=src_logits.device)
            loss_focal = focal(src_logits.transpose(1, 2), target_classes) 
...
def build(args):
    ...
    # alpha_t = json.loads(args.alpha_t)
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict,
                             eos_coef=args.eos_coef, losses=losses, focal=args.focal_loss, gamma=args.gamma, alpha_t=alpha_t)
    criterion.to(device)
