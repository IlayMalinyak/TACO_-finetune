...
def finetune():
    ...
    args = d_utils.Cfg(device=device, batch_size=2, lr=5e-5, weight_decay=1e-3, coco_path='/home/ilay.kamai/mini_project/data',
    output_dir='/home/ilay.kamai/mini_project/detr/rcnn_output')
    ...
    trainer = Trainer(model=model, optimizer=optimizer, criterion=None,
                        scheduler=scheduler, train_dl=train_dl, val_dl=val_dl,
                            device=device, optim_params=optim_param, net_params=None, nms_iou_thresh=0.4, exp_num=exp_num, log_path=args.output_dir,
                            exp_name="faster_rcnn", metric=MAP)

    model = model.to(device)
    print("trainable layers: ", trainables)
    res = trainer.fit(num_epochs=30, device=device, early_stopping=10, best='loss')
    ...
    plot_logs(log_directory,
            fields_of_interest)
    plt.savefig(f'{args.output_dir}/mAP_loss.png')
    plt.clf()

    metric = MAP(iou_threshold=[0.25,0.5,0.75], rec_thresholds=[0.25,0.5,0.75], class_metrics=True)
